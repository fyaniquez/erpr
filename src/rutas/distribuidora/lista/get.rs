//! src/rutas/distribuidora/lista/get.rs
//! author: fyaniquez
//! date: 21/10/2022
//! purpose: muestra el muestra() de lista paginada de distribuidoras

use crate::layout;
use crate::layout::lista::Paginado;
use crate::domain::distribuidora::{Distribuidora, lista_paginada};
use actix_web::get;
...
// controlador
#[tracing::instrument(name = "Lista de distribuidoras", skip(pool))]
#[get("/distribuidoras")]
pub async fn muestra(
    ...
    let (filas, total_filas) = lista_paginada(&pool, &paginado)
        .await
        .context("Error al leer distribuidoras de la BD")?;
    paginado.total_filas = Some(total_filas);

    let pagina = layout::lista::crea(
        "Distribuidoras", "/",
        "lista.css", Some("distribuidora/lista.js"),
...
// vista
fn contenido(filas: Vec<Distribuidora>) -> Option<Markup> {
    if filas.len() < 1 {
        return None;
    }
    Some(html! {
        .lista-box {
            .lista-titulo { "Distribuidoras" }
            .lista {
                .lista-cabecera {
                    span .nombre {"Nombre"}
                }
