//! src/rutas/compra/lista/get.rs
//! author: fyaniquez
//! date: 5/12/2022
//! purpose: muestra el formulario de lista paginada de compras

use crate::layout::lista;
use crate::layout::lista::Paginado;
use crate::domain::puesto::{Puesto, obtiene};
use crate::domain::compra::{Compra, lista_paginada};
use actix_web::get;
...
// controlador
#[tracing::instrument(name = "Lista de compras", skip(pool))]
#[get("/puesto/{id}/compras")]
pub async fn muestra(
    ...
    let puesto = obtiene(&pool, puesto_id).await
        .context("Error al leer punto de compra")?;
    ...
    let (filas, total_filas) = lista_paginada(&pool, &paginado, puesto_id)
        .await
        .context("Error al leer compras de la BD")?;
...
// vista
fn contenido(filas: Vec<Compra>, puesto: &Puesto) -> Option<Markup> {
    if filas.len() < 1 {
        return None;
    }
    Some(html! {
        .lista-box {
            .lista-titulo { "Compras en "(puesto.nombre) }
            .lista {
                .lista-cabecera {
                    span .nombre {"Fecha"}
                    span .nombre {"Monto"}
