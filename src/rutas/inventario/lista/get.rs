//! src/rutas/inventario/lista/get.rs
//! author: fyaniquez
//! date: 5/12/2022
//! purpose: muestra el formulario de lista paginada de inventarios

use crate::layout::lista;
use crate::layout::lista::Paginado;
use crate::domain::sucursal::{Sucursal, obtiene};
use crate::domain::inventario::{Inventario, lista_paginada};
use actix_web::get;
...
// controlador
#[tracing::instrument(name = "Lista de inventarios", skip(pool))]
#[get("/sucursal/{id}/inventarios")]
pub async fn muestra(
    ...
    let (filas, total_filas) = lista_paginada(&pool, &paginado, sucursal_id)
        .await
        .context("Error al leer inventarios de la BD")?;
...
// vista
fn contenido(filas: Vec<Inventario>, sucursal: &Sucursal) -> Option<Markup> {
    if filas.len() < 1 {
        return None;
    }
    Some(html! {
        .lista-box {
            .lista-titulo { "Inventarios en: "(sucursal.nombre) }
            .lista {
                .lista-cabecera {
                    span .nombre {"Nombre"}
                }
...
// errores considerados para lista de inventarios
