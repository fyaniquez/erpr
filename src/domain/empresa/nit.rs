//! src/domain/empresa/nit.rs
//! author: fyaniquez
//! date: 29/10/2022
//! validaciones de nit de empresa
...
impl Nit {
    /// Devuelve una instancia de NitEmpresa validado
    pub fn parse(s: String) -> Result<Nit, String> {
        ...
        if is_empty_or_whitespace || is_too_long || tiene_prohibidos {
            Err(format!("{} Nit no valido.", s))
        } else {
            Ok(Self(s))
        }
    }
}
impl AsRef<str> for Nit {
    fn as_ref(&self) -> &str {
        &self.0
    }
}
...
#[cfg(test)]
mod tests {
    use crate::domain::empresa::Nit;
    use claim::{assert_err, assert_ok};

    #[test]
    fn a_256_grapheme_long_name_is_valid() {
        let name = "a̐".repeat(128);
        assert_ok!(Nit::parse(name));
    }

    #[test]
    fn a_name_longer_than_256_graphemes_is_rejected() {
        let name = "a".repeat(129);
        assert_err!(Nit::parse(name));
    }
