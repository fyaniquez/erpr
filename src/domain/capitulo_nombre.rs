...
#[derive(Debug)]
pub struct CapituloNombre(String);

impl CapituloNombre {
    /// Devuelve una instancia de NombreCapitulo validado
    pub fn parse(s: String) -> Result<CapituloNombre, String> {
...
impl AsRef<str> for CapituloNombre {
    fn as_ref(&self) -> &str {
        &self.0
    }
}

impl std::fmt::Display for CapituloNombre {
...
#[cfg(test)]
mod tests {
    use crate::domain::CapituloNombre;
    use claim::{assert_err, assert_ok};

    #[test]
    fn a_256_grapheme_long_name_is_valid() {
        let name = "a̐".repeat(32);
        assert_ok!(CapituloNombre::parse(name));
    }

    #[test]
    fn a_name_longer_than_256_graphemes_is_rejected() {
        let name = "a".repeat(33);
        assert_err!(CapituloNombre::parse(name));
    }
